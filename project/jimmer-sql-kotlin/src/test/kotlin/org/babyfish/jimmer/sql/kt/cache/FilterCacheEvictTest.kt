...
class FilterCacheEvictTest : AbstractQueryTest() {
    ...
    fun testChangeForeignKey() {
        connectAndExpect({
            _sqlClient.binLog.accept(
                "file",
                Detector.jsonCodec().treeReader().read("{\"id\":9, \"parent_id\":8}"),
                Detector.jsonCodec().treeReader().read("{\"id\":9, \"parent_id\":2}")
            )
        }) {
    ...
    fun testChangeUserAssociations() {
        connectAndExpect({
            _sqlClient.binLog.accept(
                "file_user_mapping",
                Detector.jsonCodec().treeReader().read("{\"file_id\":28, \"user_id\":2}"),
                null
            )
        }) {
            sql(
